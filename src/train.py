...
def training_loop():
    for epoch in range(epochs):
        ...
        #if early_stopper.early_stop(validation_gen_loss):
            #break
    ...
    Path(f'D:\\Projekty\\woundprocessing\\modal_simulation\\models\\{model_name}').mkdir(parents=True, exist_ok=True)
    torch.save(generator.state_dict(), f'D:\\Projekty\\woundprocessing\\modal_simulation\\models\\{model_name}\\generator.pth')
    torch.save(discriminator.state_dict(), f'D:\\Projekty\\woundprocessing\\modal_simulation\\models\\{model_name}\\discriminator.pth')
